...
Telematic_prod_ds:= prod_ds(product_id = '109' ):INDEPENDENT;
Other_prod_ds:= prod_ds(product_id!= '109' ):INDEPENDENT;

//DistNew_base_inquiry            := DISTRIBUTE(New_base_inquiry,HASH64(lex_id,transaction_id));
//output(DistNew_base_inquiry,,'~thor::base::fcra::delta_inq_hist::20240909d::delta_key',compressed,overwrite);

dev_ds:= dataset('~thor::base::fcra::delta_inq_hist::20240909d::delta_key',lay,thor):INDEPENDENT;
Telematic_dev_ds:= dev_ds(product_id = '109' ):INDEPENDENT;
Other_dev_ds:= dev_ds(product_id!= '109' ):INDEPENDENT;
...
OUTPUT(CHOOSESETS(Other_dev_ds, lex_id!=0 AND lex_id=did => 50,
                         lex_id!=0 AND lex_id!=did => 50),named('DS_AfterChange_other_DevRecs'));

OUTPUT(COUNT(Other_prod_ds),named('Cnt_BeforeChange_other_ProdRecs'));
OUTPUT(COUNT(Other_dev_ds),named('Cnt_BeforeChange_other_DevRecs'));
...
//OUTPUT(Telematic_dev_ds(lex_id!=0),{product_id,lex_id,did},named('Telematic_dev_ds_LexNonzero'));
//OUTPUT(Other_dev_ds(lex_id!=0),{product_id,lex_id,did},named('Other_dev_ds_LexNonzero'));


//OUTPUT(CHOOSESETS(Other_dev_ds, lex_id!=0 AND lex_id=did => 50,
                         //lex_id!=0 AND lex_id!=did => 50),{product_id,lex_id,did},named('Other_dev_ds_LexNonzero'));
