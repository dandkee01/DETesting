...
Azure_File := '~thor::base::fcra::delta_inq_hist::20240917a::delta_key';

Azure_DS := DATASET(Azure_File,Lay,THOR);

OUTPUT(COUNT(Azure_DS),named('CNT_BeforeFix_Azure_DS'));

OUTPUT(Azure_DS(transaction_id='18156733R694294'),named('Azure_DS_trans94294'));
...
OUTPUT(COUNT(Today_DS),named('CNT_AfterFix_Azure_DS'));
...
Cnt := COUNT(GROUP);
END;
Azure_AfterFix  := TABLE(Today_DS,rec,lex_id,transaction_id,product_id,date_added,FEW);
OUTPUT(Azure_AfterFix(cnt>1), named('Azure_AfterFix_Transac_Count'));

rec1 := RECORD
Azure_DS.transaction_id;
Cnt := COUNT(GROUP);
END;
Azure_BeforeFix := TABLE(Azure_DS,rec1,lex_id,transaction_id,product_id,date_added,FEW);
OUTPUT(Azure_BeforeFix(cnt>1), named('Azure_BeforeFix_Transac_Count'));

OUTPUT(SORT(Azure_DS(TRIM(transaction_id,LEFT,RIGHT) IN ['18156723R686244','18156723R696236','18156723R713131','18156723R719950','18156723R738118','18156723R740116','18156723R740771','18156733R719337','18156733R721600']),transaction_id,seq_num), named('SampleTrans_BeforeFix'));    
