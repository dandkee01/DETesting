IMPORT _control, ut, Data_Services, STD;
 ...
 EXPORT FN_Copy_DataFromProd(string sname,string version, STRING Suffix) := FUNCTION
    ...
    CreateSF := IF(~ifExist,STD.FILE.CREATESUPERFILE(TILT+QA),STD.File.ClearSuperFile(TILT+QA));		
    AddSF    := STD.FILE.ADDSUPERFILE(TILT+QA,TILT+QA_Dev);
    Actions := sequential(CopyData,CreateSF,AddSF);
    Return Actions;
END;

//FN_Copy_DataFromProd('base::cdsc_trano::transaction_log_online::qa::trans_online_id' ,'20240410','base::cdsc_trano::transaction_log_online::');
